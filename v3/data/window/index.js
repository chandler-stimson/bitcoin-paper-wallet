/* global b58, bech32, secp, ripemd160 */
/*
...
const convert = {
  ...
  // converts array of 8-bit unsigned integers to an array of 5-bit unsigned
  bech32(hex) {
    // 8-bit unsigned integers
    const b8 = hex.match(/\w{1,2}/g).map(ch => (parseInt(ch, 16).toString(2)).padStart(8, '0')).join('');
    // 5-bit unsigned integers
    const b5 = b8.match(/\w{1,5}/g);
    return b5.map(b => parseInt(b, 2).toString(16).padStart(2, '0')).join('');
  }
};
...
document.getElementById('private-key-wif').addEventListener('change', async e => {
  ...
  // public key (compressed)
  {
    ...
    // native segwit
    const b32 = '00' + convert.bech32(convert.u2h(hash160));
    const segwit = bech32.encode('bc', [...convert.h2u(b32)], 'bech32');

    document.getElementById('public-address-segwit').value = segwit;
  }
