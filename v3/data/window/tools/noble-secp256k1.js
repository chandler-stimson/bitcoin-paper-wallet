...
Object.defineProperty(secp, "__esModule", { value: true });
secp.utils = secp.schnorr = secp.verify = secp.sign = secp.getSharedSecret = secp.recoverPublicKey = secp.getPublicKey = secp.SignResult = secp.Signature = secp.Point = secp.CURVE = void 0;
const CURVE = {
    a: 0n,
    b: 7n,
...
async function getQRSrfc6979(msgHash, privateKey) {
    ...
    k = await secp.utils.hmacSha256(k, v, b0, x, h1);
    v = await secp.utils.hmacSha256(k, v);
    k = await secp.utils.hmacSha256(k, v, b1, x, h1);
    v = await secp.utils.hmacSha256(k, v);
    for (let i = 0; i < 1000; i++) {
        ...
        k = await secp.utils.hmacSha256(k, v, b0);
        v = await secp.utils.hmacSha256(k, v);
    }
...
async function taggedHash(tag, ...messages) {
    ...
    const tagH = await secp.utils.sha256(tagB);
    const h = await secp.utils.sha256(concatBytes(tagH, tagH, ...messages));
...
async function schnorrSign(msgHash, privateKey, auxRand = secp.utils.randomPrivateKey()) {
    if (msgHash == null)
